...
export function addComputerZone({T,scene,box,d,white,face,dark,fabric,glow,sun}){
 ...
 const frame=[];
 for(const r of [c.frameLeft,c.frameRight]){
  for(const z of [r[1],r[3]-.02])frame.push(box('desk-floor-leg',[r[0],z,r[2],z+.02],.015,.68,steel));
  frame.push(box('desk-side-steel-rail',r,.695,.03,steel));
 }
 frame.push(box('desk-front-underrail',[c.frameLeft[0],.56,c.frameRight[2],.58],.695,.03,steel));
