"""Deterministic lighting study from the corrected live model, not a photograph."""
...
source=root/('offline-render/daylight-bake-source.glb' if bake_walls else 'offline-render/mobile-current.glb')
meta=json.loads((root/'offline-render/mobile-current-cameras.json').read_text(encoding='utf-8'))
...
for obj in bpy.data.objects:
    ...
    for slot,material in enumerate(list(obj.data.materials)):
        ...
        if obj.get('detailRole')=='sewn-textile' or material.name.startswith('textile-woven-'):
            bsdf.inputs['Sheen Weight'].default_value=.28;bsdf.inputs['Sheen Roughness'].default_value=.8
...
if len(material_audit)!=6: raise RuntimeError('Expected six explicit interior mirror surfaces in current export')
...
camera_hash=hashlib.sha256((root/'offline-render/mobile-current-cameras.json').read_bytes()).hexdigest()
