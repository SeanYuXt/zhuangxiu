...
(async()=>{
 ...
 await page.goto('http://127.0.0.1:8768/lake-home-walkthrough/master-suite-plumbing-study.html?view=footwall&v=hinged-storage-43');
 ...
 const w=a.d.wardrobeAudit;assert.equal(w.lowerLeaves,6);assert.equal(w.upperLeaves,6);assert.equal(w.drawerCount,3);
 ...
 const shot=async name=>{await page.waitForTimeout(250);await page.locator('#scene').screenshot({path:'D:/tep-chat/output/lake-home-walkthrough/master-suite-review-solid-head/hinged-storage-43-'+name+'.png'});};
 await shot('closed');
 for(const mode of ['storage','long','short','quilt','drawer','all','inside','closed']){
  ...
  if(mode==='drawer'){assert.ok(Math.abs(state.rotation-Math.PI/2)<.001);assert.equal(state.drawerZ,-.4);assert.ok((await page.locator('#route-status').textContent()).includes('占道'));}
 ...
 await page.evaluate(()=>masterPlumbing.choose('footElevation'));await page.locator('#plan').screenshot({path:'D:/tep-chat/output/lake-home-walkthrough/master-suite-review-solid-head/hinged-storage-43-elevation.png'});
 ...
 await page.locator('#plan').screenshot({path:'D:/tep-chat/output/lake-home-walkthrough/master-suite-review-solid-head/hinged-storage-43-plan-open.png'});
