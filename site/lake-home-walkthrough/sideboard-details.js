...
import {finishWoodPanel,finishQuartzPanel} from './cabinet-finishes.js';
...
export const sideboardDimensions={counterDepth:.40,upperDepth:.35,drawerBoxDepth:.30,drawerTravel:.25};

function makeSideboardShallower(cabinet,base,bank){
 // Shorten boards from the front while retaining rear planes, board thickness,
 // door bevels and appliance dimensions. The furniture group is never scaled.
 const shorten=(o,depth)=>{
  if(!o?.isMesh)throw Error('Missing sideboard panel for depth revision');
  o.geometry=o.geometry.clone();o.geometry.computeBoundingBox();
  const old=o.geometry.boundingBox.max.z-o.geometry.boundingBox.min.z;
  o.geometry.scale(1,1,depth/old);o.position.z+=(depth-old)/2;
 };
 const top=cabinet.getObjectByName('sideboard-600mm-worktop');shorten(top,.40);top.name='sideboard-400mm-worktop';
 shorten(cabinet.getObjectByName('mesh_643'),.30);
 for(const o of base.children)if(o.isMesh&&!o.name.includes('bevel')){
  o.geometry.computeBoundingBox();shorten(o,o.geometry.boundingBox.max.z-o.geometry.boundingBox.min.z-.20);
 }
 for(let i=0;i<6;i++){
  for(const root of [base,bank])root.getObjectByName('sideboard-'+(root===base?'base':'drawer')+'-bevel-'+i).position.z-=.20;
  const tray=bank.getObjectByName('sideboard-drawer-tray-'+i);
  for(const o of tray.children){o.geometry.computeBoundingBox();const depth=o.geometry.boundingBox.max.z-o.geometry.boundingBox.min.z;if(depth>.4)shorten(o,.30);else if(o.position.z>0)o.position.z-=.18;}
  tray.userData={...tray.userData,boxDepth:.30,openingTravel:.25,slideSelection:'250mm nominal travel study; hardware pending'};
 }
 shorten(cabinet.getObjectByName('mesh_716'),.36);
 for(const name of ['mesh_715','mesh_717','dispenser-concealed-body','dispenser-flush-face',...Array.from({length:16},(_,i)=>'mesh_'+(721+i))]){
  const o=cabinet.getObjectByName(name);if(!o)throw Error('Missing dispenser baseline: '+name);o.position.z-=.20;
 }
 // The existing espresso assembly is 357 mm deep including its handle. Turn
 // it along the counter so the complete, unscaled object fits the clear depth.
 const espresso=cabinet.getObjectByName('espresso-machine');espresso.rotation.y=Math.PI/2;espresso.position.z=-.088;
 for(let i=758;i<=762;i++)cabinet.getObjectByName('mesh_'+i).position.z-=.09;
 cabinet.userData={...cabinet.userData,counterDepth:.40,upperDepth:.35,upperSetback:.05,zones:['recessed-upper','400mm-workzone','independent-drawers','beveled-base'],depthRevision:'sideboard-depth-40-10',appliancesScaled:false,espressoPlacement:'Original model turned 90 degrees along the counter; product/service clearances unverified'};
}
...
function drawerContents(tray,index,x){
 ...
 if(index<2){
  for(let n=0;n<4;n++){
   const cx=x-.21+n*.14;box(content,'cutlery-partition',[cx,.704,-.10],[.010,.080,.26]);
   for(let j=0;j<3;j++){
    box(content,index?'tea-tool-grip':'cutlery-grip',[cx+.046,.670+j*.008,-.065],[.015,.005,.17],metal);
    const spoon=new T.Mesh(new T.SphereGeometry(.022,12,8),metal);spoon.name='cutlery-bowl';spoon.scale.set(.65,.10,1);spoon.position.set(cx+.046,.673+j*.008,-.163);content.add(spoon);
   }
  }
 }else if(index===2){for(const dx of [-.15,0,.15])for(const z of [-.16,-.045])place(content,makeCup('sideboard-teacup',{radius:.034,height:.067,handle:false}),[x+dx,.663,z]);}
 else if(index===3){for(let j=0;j<3;j++)box(content,'folded-table-linen',[x,.676+j*.026,-.10],[.37,.024,.25],linen);}
 else{for(const dx of [-.15,.15])box(content,index===4?'sealed-tea-box':'food-bag-organizer',[x+dx,.700,-.10],[.22,.074,.25],index===4?wood:linen);}
 return content;
}
...
export function refineSideboard(model){
 ...
 makeSideboardShallower(cabinet,base,bank);
 ...
 finishQuartzPanel(cabinet.getObjectByName('mesh_711'));
 for(let i=0;i<6;i++){
  ...
  for(const side of [-1,1]){
   box(bank,'sideboard-fixed-runner',[x+side*.305,.674,-.10],[.012,.023,.29],metal);
   box(d,'sideboard-moving-runner',[x+side*.295,.674,-.10],[.007,.019,.29],metal);
  }
  for(const [band,root,z,w] of [['base',base,.098,.646],['upper',upper,.057,.646]]){
  ...
  const low=group(base,'sideboard-lower-contents-'+i);box(low,'sideboard-adjustable-shelf',[x,.335,-.12],[.620,.018,.29]);
  if(i<3){for(const y of [.10,.345])for(let j=0;j<5;j++)place(low,makePlate('sideboard-stacked-plate',i===2?.11:.13),[x,y+j*.013,-.125]);}
  else if(i===3){for(const y of [.10,.345])for(const dx of [-.14,.14])place(low,makeCup('sideboard-breakfast-mug'),[x+dx,y,-.12]);}
  else{for(const y of [.185,.43])box(low,i===4?'tea-supply-box':'spare-table-linen-box',[x,y,-.12],[.48,.16,.26],i===4?dark:linen);}
  ...
  compartments.push({id:i,clearWidth:.632,lowerClearDepth:.29,upperClearDepth:.286,upperTierClearHeights:[.298,.283],drawerUseful:[.564,.118,.276],contents:['餐具','茶工具','茶杯','桌布餐巾','茶叶盒','保鲜用品'][i]});
 }
 ...
 const cap=cabinet.getObjectByName('mesh_718');cap.geometry=cap.geometry.clone();cap.geometry.scale(1,1,.351/.56);cap.position.z=-.1045;finishWoodPanel(cap,{interior:true,refresh:true});
 const pierCase=group(cabinet,'dispenser-service-pier');
 box(pierCase,'service-pier-left',[1.306,1.3275,-.1045],[.012,.845,.351]);
 box(pierCase,'service-pier-back',[1.36,1.3275,-.274],[.096,.845,.012]);
 for(const y of [.911,1.744])box(pierCase,'service-pier-end',[1.36,y,-.10],[.096,.012,.336]);
 // Pipe/cable penetration reserves in the inside face; cabinet only, no wall drilling.
 const ys=[.905,1.36,1.38,1.59,1.61,1.75],zs=[-.28,-.19,-.17,-.15,-.13,.071];
 ...
 box(pierCase,'service-pier-wet-dry-divider',[1.36,1.30,-.025],[.096,.012,.186]);
 for(const y of [1.053,1.087])box(pierCase,'service-valve-mount',[1.36,y,-.0155],[.096,.008,.016],metal);
 for(const y of [1.57,1.63])box(pierCase,'service-power-mount',[1.36,y,.008],[.096,.012,.018],metal);
 const serviceFace=group(cabinet,'dispenser-service-door',[1.30,0,.098]);cabinet.updateMatrixWorld(true);serviceFace.attach(pierFront);
 ...
 box(service,'dispenser-isolation-valve',[1.36,1.07,.02],[.045,.047,.055],metal);
 box(service,'dispenser-water-valve-lever',[1.36,1.096,.045],[.063,.012,.025],dark);
 box(service,'dispenser-capped-feed-port',[1.36,1.10,.01],[.018,.026,.026],dark);
 box(service,'dispenser-dry-power-reserve',[1.36,1.60,.025],[.068,.074,.016],linen);
 box(service,'dispenser-power-splash-cover',[1.36,1.60,.043],[.076,.084,.015],dark);
 box(service,'dispenser-service-leak-sensor',[1.36,.924,0],[.044,.014,.038],linen);
 for(const [name,pts,r] of [
  ['dispenser-capped-water-tail',[[1.36,1.07,-.005],[1.36,1.07,-.14],[1.36,1.37,-.14],[1.51,1.37,-.14],[1.51,1.37,-.218]],.003],
  ['dispenser-unconnected-cable-tail',[[1.36,1.60,.015],[1.36,1.60,-.18],[1.70,1.60,-.18],[1.70,1.60,-.218]],.0035]
 ...
 function setDrawer(index,fraction=1){drawers.forEach((g,i)=>g.position.z=i===index?sideboardDimensions.drawerTravel*T.MathUtils.clamp(fraction,0,1):0);state.drawer=index;model.updateMatrixWorld(true);}
