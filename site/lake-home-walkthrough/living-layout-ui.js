...
import {wireSeatUse} from './living-seat-use.js?v=balcony-right-utility-3';
...
export function wireLivingLayout(model,onChange=()=>{}){
 ...
 Object.assign(metrics,{rightWingToCare:care.min.x-right.max.x,barDepth:right.max.z-right.min.z,glassToTable:left.min.z-.1,occupiedSeatToSofa:sofa.min.z-(seatCenter.z+.45+.325)});
 ...
 rect(box('balcony-tool-cabinet'),'#cbbd9f','工具');label(6.9,.8,'休闲娱乐',undefined,.15);
 rect(box('balcony-wet-hamper'),'#d9d3c5','盆柜');rect(box('tv-display-lake'),'#c9b896','展示');
 ...
 dialog.querySelector('.layout-metrics').innerHTML=`<p>双翼各 ${Math.round(spec.wingLengths.left*100)} cm、每侧两席（座位中心距65cm）；沙发—岛台净宽 ${Math.round(metrics.islandToSofa*100)} cm；岛台—厨房开口所在墙完成面约 ${Math.round(metrics.islandToKitchenWall*100)} cm（不是门洞宽，当前模型门洞 ${Math.round(metrics.kitchenOpening*100)} cm）。</p><p>坐姿眼位至屏幕平面约 ${normalDistance.toFixed(2)} m，水平视角约 ${horizontalAngle.toFixed(0)}°。眼高暂按 1.08 m，偏沉浸的 85 寸 4K 观看方案；不是唯一最佳距离。沙发—茶几约 ${Math.round(metrics.sofaToCoffee*100)} cm。</p><p>浅米灰细织短绒地毯 220 × 320 cm，长边沿沙发；压住沙发前部，覆盖茶几，避开餐岛主通道。防滑底、扫地机爬毯与成品尺寸待选型。</p><p>洗烘柜与右翼桌端横向约 ${Math.round(metrics.rightWingToLaundry*100)} cm，但机门区与桌深错位，不能只看这一个横向数值。开门侧邻近站位的简化代理找到通路，不等于弯腰取衣、完整开门扫掠和检修已验收。原房门保留，柱位按1450mm净深校正；给排水走向未设计。</p>`;
 ...
 const powerNote=document.createElement('p');powerNote.textContent='左侧办公与茶席、右侧叠衣；浅暖灰包柱，去掉明装板；桌面同色翻盖电源盒经饰面背后和桌下检修槽预留，不凿承重柱、不跨地面走线。USB-C PD 65–100W是选型目标；烧水器具不串接办公排插。供电起点、同时负载、回路/线径、接地、漏电保护及防溅由电气设计和机型核定，图中未接电。';dialog.append(powerNote);
 const sideNote=document.createElement('p');sideNote.textContent='阳台净深1450mm按用户值试排：右侧550mm盆柜（下放上下水基站）＋660mm洗烘＋180mm长物挂架，共1390mm，窗侧/柱线侧各余30mm。柱梁保留、顶面原高。';dialog.append(sideNote);
 const careNote=document.createElement('p');careNote.textContent='左侧原清洁柜全部撤掉，保留休闲与娱乐。右侧卫生生活区使用统一720mm柜深，柜后给排水未接现场。电视低柜截短至2750mm，止于独立展示柜，接缝明确。';dialog.append(careNote);
