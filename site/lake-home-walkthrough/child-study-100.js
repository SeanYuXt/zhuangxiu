...
const d=await(await fetch('./child-study-100.json')).json(),host=document.querySelector('#viewer'),canvas=document.querySelector('canvas'),svg=document.querySelector('#plan');
...
for(const z of[.02,.72])box('wardrobe-side',[0,z,.50,z+.018],.06,2.44,white);
for(const y of[.06,2.482])box('wardrobe-shelf',[0,.02,.50,.738],y,.018,white);
...
curvedVolume('rounded-wardrobe-end',[0,.738,.50,1.22],.085,2.395,.10,white);
box('wardrobe-door-reveal',[.499,.96,.501,.966],.12,2.30,wood);
...
box('mini-pc',[.06,.40,.24,.58],1.148,.16,white);
...
// One high shallow cabinet; the right side stays open above the bed.
const u=d.upperCabinet;curvedFront('small-curved-upper',u.rect[0],u.bottom,u.rect[1],.70,u.height,.22,u.radius,white);
box('upper-door-seam',[.885,.247,.891,.249],1.89,.40,wood);
curvedVolume('light-open-shelf',[.55,.035,1.24,.215],1.50,.020,.035,wood);
for(let i=0;i<6;i++)box('books',[.60+i*.034,.055,.624+i*.034,.20],1.522,.23,i%2?blue:linen);
box('concealed-desk-light',[.60,.19,1.18,.20],1.488,.008,glow);
...
let pulled=false,open=true;function update(){const p=d.chair[pulled?'seated':'parked'];chair.position.set(p[0],0,p[1]);chair.rotation.y=-Math.PI/2;pivot.rotation.y=open?-Math.PI/2:0;document.querySelector('#chair').textContent=pulled?'收回椅子':'拉出椅子';document.querySelector('#door').textContent=open?'关门':'开门';document.querySelector('#note').textContent='圆弧衣柜收口 · 悬浮桌板穿柜，靠床预留28cm床头置物区 · 上柜减至70cm宽、22cm深，底部抬至185cm。支撑与柜门五金待深化。';plan();}
