...
const d=await(await fetch('./child-study-101.json?v=foot-wall')).json(),host=document.querySelector('#viewer'),canvas=document.querySelector('canvas'),svg=document.querySelector('#plan');
...
let pulled=false,open=true;function update(){const p=d.chair[pulled?'seated':'parked'];chair.position.set(p[0],0,p[1]);chair.rotation.y=-Math.PI/2;pivot.rotation.y=open?-Math.PI/2:0;document.querySelector('#chair').textContent=pulled?'收回椅子':'拉出椅子';document.querySelector('#door').textContent=open?'关门':'开门';document.querySelector('#note').textContent='三层书格，净层高约32cm · 高处封闭柜，床头上方留白 · 空调移至床尾墙，预留孔/排水/送风待核。悬浮支撑待深化。';plan();}
