...
(async()=>{
 ...
 await page.goto('http://127.0.0.1:8768/lake-home-walkthrough/master-suite-plumbing-study.html?view=footwall&v=split-drawers-46');
 ...
 assert.equal(a.s.design.footCabinet.drawers.filter(d=>d.accent).length,1);
 const ds=a.s.design.footCabinet.drawers;assert.equal(ds[1].base,ds[2].base);assert.ok(ds[1].front[2]<ds[2].front[0]);assert.ok(ds[0].base+ds[0].height<ds[1].base);assert.ok(ds.every(d=>d.base+d.height-.035>=.95&&d.base+d.height-.035<1.12));
 ...
 const shot=async name=>{await page.waitForTimeout(250);await page.locator('#scene').screenshot({path:'D:/tep-chat/output/lake-home-walkthrough/master-suite-review-solid-head/split-drawers-46-'+name+'.png'});};
 await shot('closed');
 ...
 await page.evaluate(()=>masterPlumbing.choose('footElevation'));await page.locator('#plan').screenshot({path:'D:/tep-chat/output/lake-home-walkthrough/master-suite-review-solid-head/split-drawers-46-elevation.png'});
 ...
 await page.locator('#plan').screenshot({path:'D:/tep-chat/output/lake-home-walkthrough/master-suite-review-solid-head/split-drawers-46-plan-open.png'});
